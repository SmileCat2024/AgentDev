...
import type { QQBotAgentAdapter, QQBotInboundAttachment, QQBotInboundRequest } from "./agent.js";
...
import { getQQBotAgentAdapter } from "./agent.js";
import { createOpenClawAgentAdapter } from "./openclaw-agent-adapter.js";
import { hasQQBotRuntime } from "./runtime.js";
...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  const agentAdapter = ctx.agentAdapter ?? getQQBotAgentAdapter() ?? (hasQQBotRuntime() ? createOpenClawAgentAdapter() : null);

  if (!account.appId || !account.clientSecret) {
    throw new Error("QQBot not configured (missing appId or clientSecret)");
  }
  if (!agentAdapter) {
    throw new Error("QQBot agent adapter not configured. Call setQQBotAgentAdapter(...) or use the OpenClaw plugin runtime.");
  }
  ...
  const connect = async () => {
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        const normalizedAttachments: QQBotInboundAttachment[] = [];
        ...
        const request: QQBotInboundRequest = {
          channel: "qqbot",
          accountId: account.accountId,
        ...
        try {
          ...
          const dispatchPromise = agentAdapter.handleMessage({
            request,
            account,
            cfg,
            abortSignal,
          ...
          try {
            ...
            if (!hasResponse) {
              const errMsg = String(err);
              if (errMsg !== "Error: Response timeout") {
