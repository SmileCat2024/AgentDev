/**
 * @deprecated 使用 DebugHub 和 ViewerWorker 代替
 * 此文件保留用于向后兼容，实际功能已迁移到 viewer-worker.ts
 */

/**
 * 占位符类 - 仅用于向后兼容
 * 请使用 DebugHub.getInstance() 代替
 */
export class MessageViewer {
  constructor() {
    throw new Error('MessageViewer is deprecated. Use DebugHub.getInstance() instead.');
  }
}
