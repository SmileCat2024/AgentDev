    ...
    function updateHookPanelOpenState(kind, key, isOpen) {
      if (kind === 'features') {
        hookPanelOpenState.features = !!isOpen;
        return;
      }
      hookPanelOpenState.lifecycles[key] = !!isOpen;
    }

    window.updateHookPanelOpenState = updateHookPanelOpenState;
    ...
    function renderHookPanel() {
      ...
      const flowChips = currentHookInspector.lifecycleOrder
        .map(name => '<div class="hooks-chip"><strong>' + escapeHtml(name) + '</strong></div>')
        .join('');
      ...
      const lifecycleCards = currentHookInspector.hooks
        .map(group => {
          const isOpen = Object.prototype.hasOwnProperty.call(hookPanelOpenState.lifecycles, group.lifecycle)
            ? hookPanelOpenState.lifecycles[group.lifecycle]
            : group.kind === 'decision';
          ...
          return [
            '<details class="hook-lifecycle-card"' + (isOpen ? ' open' : '') + ' ontoggle="window.updateHookPanelOpenState('lifecycle', '' + escapeHtml(group.lifecycle) + '', this.open)">',
            '<summary class="hook-lifecycle-head">',
            ...
            '<div class="hook-lifecycle-toggle">›</div>',
            '</div>',
            '</summary>',
      ...
      if (currentHookInspector.features.length === 0 && currentHookInspector.hooks.length === 0) {
      ...
      return [
        ...
        '<div class="hooks-hero-subtitle">Shows enabled features and the actual reverse-hook chain currently registered on this agent.</div>',
        '<div class="hooks-stats">',
        '<div class="hooks-stat"><div class="hooks-stat-label">Enabled Features</div><div class="hooks-stat-value">' + String(currentHookInspector.features.filter(feature => feature.enabled).length) + '</div></div>',
        ...
        '<div class="hooks-section-header"><div class="hooks-section-title">Loop Flow</div><div class="hooks-section-meta">Execution order preview</div></div>',
        '<div class="hooks-strip">' + flowChips + '</div>',
        '</section>',
        '<details class="hooks-collapsible"' + (hookPanelOpenState.features ? ' open' : '') + ' ontoggle="window.updateHookPanelOpenState('features', 'features', this.open)">',
        '<summary class="hooks-section-header" style="padding:12px 14px;">',
        '<div class="hooks-section-title">Enabled Features</div><div class="hooks-section-meta">Current runtime snapshot</div>',
        '</summary>',
        '<div class="hooks-collapsible-body">',
        '<div class="feature-grid">' + featureCards + '</div>',
        '</div>',
        '</details>',
        '<section class="hooks-section">',
        '<div class="hooks-section-header"><div class="hooks-section-title">Reverse Hook Chain</div><div class="hooks-section-meta">Decision hooks opened by default</div></div>',
