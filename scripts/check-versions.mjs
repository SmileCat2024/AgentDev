 ...
 *
 * 良好的锁步纪律也要求这些包对 `@agentdevjs/core` 的依赖版本与自身版本
...
function main() {
  ...
  for (const name of LOCKSTEP) {
    ...
    const coreRange = deps['@agentdevjs/core'];
    ...
    if (!coreRange.includes(prefix)) {
      fail(`${name} 对 @agentdevjs/core 的依赖 range "${coreRange}" 与锁步版本 ${expected} 错位`);
    } else {
      ok(`${name} 的 @agentdevjs/core 依赖 range "${coreRange}" 对齐锁步版本`);
    }
  }
