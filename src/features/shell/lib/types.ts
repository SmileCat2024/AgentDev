...
/**
 * 删除模式（复用自 trash-cli.put.core.mode）
 */
export enum Mode {
...
/**
 * 删除结果（复用自 trash-cli.put.core.trash_result）
 */
...
/**
 * 批量删除结果（复用自 trash-cli.put.core.trash_all_result）
 */
export interface TrashAllResult {
  failedPaths: string[];
  anyFailure(): boolean;
}
...
export interface TrashedFileInfo {
  ...
  deletionDate: string;       // 删除日期时间
  operator: string | null;    // 操作者
  trashFile: string;          // 垃圾文件路径
